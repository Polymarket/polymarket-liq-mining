...
describe("MerkleDistributor", () => {
  ...
  describe("#claim", () => {
    ...
    it("fails when frozen", async () => {
      ...
      await expect(
        distributor.claim(0, wallet0.address, 0, [ZERO_BYTES32], overrides)
    ...
    describe("two account tree", () => {
      ...
      it("increments totalClaimed", async () => {
        expect(await distributor.totalClaimed(wallet0.address)).to.eq(0);
        const proof0 = tree.getProof(0, wallet0.address, BigNumber.from(100));
        await distributor.claim(0, wallet0.address, 100, proof0, overrides);
        expect(await distributor.totalClaimed(wallet0.address)).to.eq(100);
      });
  ...
  describe("#freeze", () => {
    ...
    it("fails if not called by owner", async () => {
      ...
      expect(distributorFromOtherWallet.freeze()).to.be.revertedWith(
  ...
  describe("#unfreeze", () => {
    ...
    it("fails if not called by owner", async () => {
      ...
      const distributorFromOtherWallet = distributor.connect(wallet1);
      expect(distributorFromOtherWallet.unfreeze()).to.be.revertedWith(
        "Ownable: caller is not the owner"
      );
    });
  });
  ...
  describe("#updateMerkleRoot", () => {
    it("fails when not frozen", async () => {
      ...
      await expect(
        distributor.updateMerkleRoot(ONE_BYTES32)
      ).to.be.revertedWith("MerkleDistributor: Contract not frozen.");
    });
    ...
    it("fails if not called by owner", async () => {
      ...
      const distributorFromOtherWallet = distributor.connect(wallet1);
      expect(
        distributorFromOtherWallet.updateMerkleRoot(ONE_BYTES32)
  ...
  describe("parseBalanceMap", () => {
    ...
    it("check the proofs is as expected", () => {
      // addresses are sorted in parseBalanceMap. then random claim indexes are created.
      // wallet0 is not necessarily < wallet1, so sort them here...
      // if this test fails, the wallet addresses changed. so hardcode the new proofs
      // ...at least the address order, index, amount should be correct...
      const expected = [
        { amount: 200, address: wallet0.address },
        { amount: 250, address: wallets[2].address },
        { amount: 300, address: wallet1.address },
      ].sort((a, b) => a.address - b.address);

      expect(claims).to.deep.eq({
        [expected[0].address]: {
          index: 0,
          amount: BigNumber.from(expected[0].amount).toHexString(),
          proof: [
        ...
        [expected[1].address]: {
          index: 1,
          amount: BigNumber.from(expected[1].amount).toHexString(),
          proof: [
        ...
        [expected[2].address]: {
          index: 2,
          amount: BigNumber.from(expected[2].amount).toHexString(),
          proof: [
