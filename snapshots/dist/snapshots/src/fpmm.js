...
const calculateValOfLpPositions = async (marketAddress, block) => {
    const fpmm = await (0, exports.getFpmm)(marketAddress, block);
...
const getFpmm = async (marketAddress, block) => {
    ...
    while (retryCount < tries) {
        try {
            const { data } = await (0, gql_client_1.queryGqlClient)(queries_1.getFixedProductMarketMakerQuery, {
...
const calculateValOfLpPositionsWrapper = async (args) => {
    return await (0, exports.calculateValOfLpPositions)(args.marketAddress, args.block);
};
const calculateValOfLpPositionsBatched = (0, promises_tho_1.batch)({ batchSize: 75 }, calculateValOfLpPositionsWrapper);
