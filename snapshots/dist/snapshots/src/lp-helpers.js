    ...
    const liqMap = epochInfo.reward_markets.reduce((acc, curr) => {
        curr.reward_tokens_liquidity.forEach((token) => {
            ...
            if (rewardMarketStartDate &&
                rewardMarketEndDate &&
                eventStartDate) {
                (0, exports.validateEventStartBlock)(rewardMarketStartDate, eventStartDate, rewardMarketEndDate, marketMaker);
            }
...
const updateTokensPerBlockReward = (userTokensPerEpoch, liquidityAcrossBlocks, perBlockReward) => {
    ...
    for (const liquidityAtBlock of liquidityAcrossBlocks) {
        const sumOfBlockLiquidity = (0, helpers_1.sumValues)(liquidityAtBlock);
        const blockPoints = (0, helpers_1.makePayoutsMap)(liquidityAtBlock, sumOfBlockLiquidity, perBlockReward);
        updatedMap = (0, helpers_1.combineMaps)([blockPoints, updatedMap]);
    }
    return updatedMap;
};
