...
const getAllFeesInEpoch = async (startTimestamp, endTimestamp) => {
    ...
    while (retryCount < tries) {
        try {
            let lastId = "";
            const search = true;
            const startTimestampInSeconds = utils_1.normalizeTimestamp(startTimestamp);
            const endTimestampInSeconds = utils_1.normalizeTimestamp(endTimestamp);
            const usersWithTransactions = [];
            while (search) {
                //Subgraph can only pull 1k accounts at a time,
                const { data } = await gql_client_1.queryGqlClient(queries.getUsersWhoPaidFeesInEpochQuery, {
