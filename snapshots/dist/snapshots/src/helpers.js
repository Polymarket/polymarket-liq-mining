...
const addEoaToUserPayoutMap = async (map) => {
    ...
    return Promise.all(Object.keys(map).map(async (userAddress) => {
        const eoaWallet = await (0, eoa_1.getEoaLinkAddress)(userAddress);
        return {
...
const formatEstimatedRewards = (map, epoch, reward_token, isUSDC) => {
    return positiveAddressesOnly(map).map((username) => {
        return {
            username: (0, exports.validateAddress)(username),
            epoch,
            reward_token,
            estimated_liq: (0, exports.getAmountInEther)(map[username], isUSDC).toHexString(),
        };
    });
};
...
const normalizeEarningsNewFormat = (map, isUSDC) => {
    return positiveAddressesOnly(map).map((addr) => {
        return {
            address: (0, exports.validateAddress)(addr),
            earnings: bignumber_1.BigNumber.isBigNumber(map[addr])
                ? map[addr].toString()
                : (0, exports.getAmountInEther)(map[addr], isUSDC).toString(),
            reasons: "",
        };
    });
};
...
const normalizeMapForMultipleEpochs = (map, isUSDC) => {
    return positiveAddressesOnly(map).reduce((acc, curr) => {
        ...
        if (!acc[lowerCase]) {
            acc[lowerCase] = (0, exports.getAmountInEther)(map[curr], isUSDC);
        }
        return acc;
    }, {});
};
...
const combineMerkleInfo = (prevClaims, newClaimMap, isUSDC) => {
    ...
    const newMap = (0, exports.normalizeMapForMultipleEpochs)(newClaimMap, isUSDC);
    const combined = (0, exports.combineBigNumberMaps)([newMap, mapOfUnpaidClaims]);
    const normalized = (0, exports.normalizeEarningsNewFormat)(combined, isUSDC);
    return (0, parse_balance_map_1.parseBalanceMap)(normalized);
};
