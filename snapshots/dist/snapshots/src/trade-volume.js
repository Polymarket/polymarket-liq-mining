...
const getTradeVolumePerUser = async (address, timestamp) => {
    ...
    while (retryCount < tries) {
        try {
            ...
            const search = true;
            while (search) {
                const { data } = await (0, gql_client_1.queryGqlClient)(queries.getTradeVolumePerUserQuery, {
                    lastId: lastId,
                    user: address,
                    timestamp: `${timestamp}`,
                });
...
const getFeesPerUser = async (address, startTimestamp, endTimestamp) => {
    ...
    while (retryCount < tries) {
        try {
            ...
            const search = true;
            while (search) {
                const { data } = await (0, gql_client_1.queryGqlClient)(queries.getFeesPaidPerUserQuery, {
                    lastId: lastId,
                    user: address,
...
const getTradeVolumeBatched = (0, promises_tho_1.batch)({ batchSize: 100 }, getTradeVolumePerUserWrapper);
const getFeesBatched = (0, promises_tho_1.batch)({ batchSize: 100 }, getFeesPerUserWrapper);
/**
