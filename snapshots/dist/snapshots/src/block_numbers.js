...
async function getFirstAddedLiquidity(marketAddress) {
    ...
    while (retryCount < tries) {
        try {
            const { data } = await (0, gql_client_1.queryGqlClient)(queries_1.firstLiquidityAddedQuery, {
...
async function getMarketResolutionTransaction(marketAddress) {
    ...
    while (retryCount < tries) {
        try {
            const { data } = await (0, gql_client_1.queryGqlClient)(queries_1.marketResolutionTxnQuery, {
                market: marketAddress,
            });
...
async function convertTimestampToBlockNumber(timestamp) {
    ...
    const nowInSeconds = (0, utils_1.normalizeTimestamp)(Date.now());
