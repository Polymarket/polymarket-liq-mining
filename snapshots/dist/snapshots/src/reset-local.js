...
async function resetLocal(collection) {
    ...
    try {
        ...
        const recordResponse = await cross_fetch_1.default(`${constants_1.LOCAL_STRAPI_URL}/reward-${endpointSuffix}?_limit=-1`);
        ...
        await cross_fetch_1.default(`${constants_1.LOCAL_STRAPI_URL}/reward-${endpointSuffix}/${firstOne.id}`, {
            method: "DELETE",
            headers: {
        ...
        while (recordsJson.length > 0) {
            ...
            await Promise.all(batch.map(async (each, index) => {
                ...
                await cross_fetch_1.default(`${constants_1.LOCAL_STRAPI_URL}/reward-users/${each.id}`, {
                    method: "DELETE",
                    headers: {
                        "Authorization": `Bearer ${token}`
                    }
