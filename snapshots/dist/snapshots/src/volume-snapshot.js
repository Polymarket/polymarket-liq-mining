...
async function generateVolumeSnapshot(timestamp, supply) {
    ...
    // get all users
    const users = await (0, users_1.getAllUsers)(timestamp);
    //get volume per user at the timestamp
    console.log(`Fetching trade volume per user at snapshot...`);
    const tradeVolumes = await (0, trade_volume_1.getTradeVolume)(users, timestamp);
    // get total volume
    ...
    for (const userIndex in users) {
        ...
        if (userVolume > 0) {
            ...
            const eoa = await (0, eoa_1.fetchEoaAddress)(user);
