...
(async () => {
    ...
    const validEnvVars = await (0, validate_env_vars_1.validateEnvVars)(CHECK_ENV_VARS);
    if (!validEnvVars) {
    ...
    let chosenEpoch;
    try {
        const numRes = await (0, cross_fetch_1.default)(`${constants_1.STRAPI_URL}/reward-epoches/count?is_active=true`);
    ...
    const epochRes = await (0, cross_fetch_1.default)(`${constants_1.STRAPI_URL}/reward-epoches/${chosenEpoch}`);
    const epochInfo = await epochRes.json();
    console.log("epochInfo", epochInfo);
    (0, lp_helpers_1.ensureGoodDataFromStrapi)(epochInfo);
    const { startTimestamp, endTimestamp, tokenMap } = (0, lp_helpers_1.cleanAndSeparateEpochPerToken)(epochInfo);
    ...
    for (const tokenId of Object.keys(tokenMap)) {
        ...
        const tokenDataResponse = await (0, cross_fetch_1.default)(`${constants_1.STRAPI_URL}/reward-tokens/${tokenId}`);
        ...
        // ------------------------------------------------
        // const t1 = Date.now();
        const liqMap = await (0, lp_snapshot_1.generateLpSnapshot)(startTimestamp, endTimestamp, markets, Number(constants_1.DEFAULT_BLOCKS_PER_SAMPLE), false);
        // console.log(`${tokenId} liqMap`, liqMap);
        console.log(`${tokenId} liqMap`, Object.keys(liqMap).length + " liquidity providers");
        const estimatedRewards = (0, helpers_1.formatEstimatedRewards)(liqMap, chosenEpoch, Number(tokenId), isUSDC);
        ...
        while (estimatedRewards.length > 0) {
            ...
            try {
                ...
                const response = await (0, cross_fetch_1.default)(`${constants_1.STRAPI_URL}/reward-users`, {
                    method: "PUT",
                    headers: {
                        "Content-Type": "application/json",
