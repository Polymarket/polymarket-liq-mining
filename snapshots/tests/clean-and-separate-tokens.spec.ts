...
describe("clean and separate epoch per token", () => {
            ...
            reward_tokens: [
                {
                    ...
                    clob_liqudity_token_supply: "20000",
                },
                {
                    id: 2,
                    reward_token: {
                    ...
                    clob_liqudity_token_supply: "10000",
                },
            ],
            reward_markets: [
                {
    ...
    it("should work", () => {
        ...
        expect(x.tokenMap["1"].clobLiqSupply).to.eq(
            BigNumber.from("20000").toNumber(),
        );
        expect(x.tokenMap["2"].clobLiqSupply).to.eq(
            BigNumber.from("10000").toNumber(),
        );
    });
    ...
    it("should not fail if only adding an extra token for fees", () => {
        const thirdToken = {
            ...
            clob_liqudity_token_supply: "300000",
        };
        ...
        expect(expected.tokenMap[token3Id].clobLiqSupply).to.eq(
            BigNumber.from("300000").toNumber(),
        );
    });
    ...
    it("should not fail if only adding an extra liq token to an existing market", () => {
        ...
        const expected = cleanAndSeparateEpochPerToken(mockEpochInfo);
        expect(expected.tokenMap[token3Id].markets.length).to.eq(1);
        expect(expected.tokenMap[token3Id].markets[0].amount).to.eq(
